...
matrix::Matrix<float, 11, 1> AOBLTR::evaluateRBF(const float rate) {
...
void AOBLTR::calculateAdaptation(const float rate) {
	
	matrix::Matrix<float, 11, 1> torque_rbf = evaluateRBF(rate);

	for(int i = 0; i < 11; ++i) {
		_torque_bar(i,0) = torque_rbf(i,0);
	}
	_torque_bar(11, 0) = _baseline_torque;

	float adaptation_torque = -(_torque_theta_hat.T() * _torque_bar)(0,0);
	_adaptation_torque = adaptation_torque;
}

void AOBLTR::updateWeights(const float rate, const float rate_int, const float dt) {
	...
	matrix::Matrix<float, 2, 1> e_state = current_rate_state - x_hat;
	
	float mu = muDeadzone(e_state, dt);
	matrix::Matrix<float, 2, 1> eDeadzone_torque = e_state * mu;
	matrix::Matrix<float, 1, 2> eDeadzone_T = eDeadzone_torque.transpose();  // 1x2
	matrix::Matrix<float, 12, 2> tmp_12x2 = _torque_bar * eDeadzone_T;   // 12x2
	matrix::Matrix<float, 2, 2> _P_lyap_surge = _filter.getLyap();
	matrix::SquareMatrix<float, 2> _P_lyap_surge_square = _P_lyap_surge;
	matrix::Matrix<float, 2, 2> _P_lyap_surge_INV = _P_lyap_surge_square.I();
	matrix::Matrix<float, 12, 2> tmp_12x2b = tmp_12x2 * _P_lyap_surge_INV;   // 12x2
	matrix::Matrix<float, 2, 1> _B_torque_ol_ext;
	_B_torque_ol_ext.setZero();
	_B_torque_ol_ext(1,0) = _B_torque_ol;
	matrix::Matrix<float, 12, 1> tmp_12x1 = tmp_12x2b * _B_torque_ol_ext;     // 12x1:
	
	matrix::Matrix<float, 12, 1> torque_theta_hat_dot = -_theta_gain_torque * tmp_12x1;
...
matrix::Matrix<float, 12, 1> AOBLTR::rectangularProjection(
...
float AOBLTR::muDeadzone(const matrix::Matrix<float, 2, 1> &e, const float dt) {
...
float AOBLTR::update(float rate,
                  float rate_error,
                  float rate_int,
                  float rate_int_prev,
                  float angular_accel,
