...
public:
	...
	matrix::Matrix<float, 12, 1> rectangularProjection(const matrix::Matrix<float, 12, 1> &thetaHat,
							const matrix::Matrix<float, 12, 1> &thetaHatDot,
							float epsTheta, float thetaHatMax, 
							float thetaHatMin, float dt);
...
private:
	...
	float _eNullDeadzone{0.5f}; //tunable param
	float _B_torque_ol{0.04f}; //tunable param
	float _theta_gain_torque{0.00005f}; //tunable param
	float _eps_theta_torque{0.0003}; //tunable param
	float _theta_hat_max_torque{1.0f}; //tunable param
	float _theta_hat_min_torque{-1.0f}; //tunable param
